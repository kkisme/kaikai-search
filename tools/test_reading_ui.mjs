...
try{
 ...
 assert((await page.locator('#installGuide').innerText()).includes('本系统由 刘凯&AI 开发'));
 ...
 assert(await page.locator('#clearSearch').isHidden());
 ...
 assert(await page.locator('#clearSearch').isHidden());
 assert(await page.locator('#searchInput').evaluate(el=>el===document.activeElement));
 await page.locator('#searchInput').dispatchEvent('compositionstart');
 await page.locator('#searchInput').fill('事故');
 await page.locator('.search-submit').click();
 assert(await page.locator('#matchNav').isVisible(),'search navigation must show after submitting during IME composition');
 assert((await page.locator('#matchCount').textContent()).startsWith('1 / '));
 assert((await page.locator('#stat').textContent()).startsWith('找到'));
 await page.screenshot({path:'dist/search-navigation-mobile.png'});
 await page.locator('#clearSearch').click();
