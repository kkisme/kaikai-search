...
面向多个题库的移动端阅读与搜索工具，网页入口：https://kkisme.github.io/kaikai-search/ 。当前收录的第一个题库是“交安复习题”。

当前题库以 `交安复习题全部.大题层级修正版-20260907.docx` 的 1206 个独立条目为基础，加入 2026-09-24 两份新文档中可完整识别且不重复的 1069 条，共 2275 条。原有案例背景、题内说明和小题按文档顺序保留；搜索命中一个小题也展示完整案例。新文档中的残缺题暂缓收录，详情见 `交安题库新增核对-20260924.md`。
...
python tools/import_20260924.py
python -m http.server 8000 --directory docs
```

打开 http://localhost:8000/ 。重建时先生成基础题库，再运行新增题导入脚本；新文档放在 `data/` 下。基础题生成支持 `--source 文件路径`。`docs/question-bank.json` 包含来源文档及 SHA-256，方便追溯；旧解析缓存 JSON 不参与页面读取。
